...
(function() {
  ...
  // ------------------------------
    ...
    // 测试 API 连通性
        const $btnTestApi = root$4.querySelector("#ykt-btn-test-api");
    const $testResult = root$4.querySelector("#ykt-api-test-result");
    if ($btnTestApi && $testResult) $btnTestApi.addEventListener("click", () => {
  ...
  /**
   * 通用 OpenAI 协议文本模型调用
   */  async function queryAI(question, aiCfg) {
 ...
 // ★ 标记为已完成
  ...
  function updateRow(row, e, prob) {
    ...
    if (pType === 1 || pType === 3) {
      ...
      if (existingSub) textarea.value = typeof existingSub === "string" ? existingSub : existingSub.content || "";
    ...
    // 从当前输入获取 result 的统一入口（优先高级模式的 JSON）
      ...
      if (advBox.style.display !== "none" && advTextarea.value.trim()) return JSON.parse(advTextarea.value);
    ...
    // AI 补交：调用 AI 生成答案 → 填入输入 → 自动补交
        const btnAIRetry = create("button", "retry-ai-btn");
    ...
    btnAIRetry.onclick = async () => {
      ...
      try {
        const q = formatProblemForAI(prob || {
          problemId: e.problemId,
          problemType: pType,
          body: e.body || "",
          options: options
  ...
  function fillInputsFromResult(result, pType, editorBox, problemId, options) {
    if ((pType === 1 || pType === 3) && Array.isArray(result)) {
    ...
    } else if (pType === 2 && Array.isArray(result)) if (options.length > 0) editorBox.querySelectorAll('.retry-options-group input[type="checkbox"]').forEach(cb => {
      ...
      const input = editorBox.querySelector(".retry-text-input");
      if (input) input.value = result.join(",");
    } else if (pType === 5 || typeof result === "object" && result.content !== void 0) {
